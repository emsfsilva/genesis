...
router.get('/edit', eAdmin, async (req, res) => {
    ...
    if (user) {
        ...
        return res.render('unidade/unidadeprofile/edit', { layout: 'main', profile: req.user.dataValues, data: dataForm });
    } else {
        req.flash("danger_msg", "Erro: Usuário não encontrado!");
        return res.redirect('/login');
    }
...
router.post('/edit', eAdmin, async (req, res) => {
    ...
    db.users.update(data, { where: { id: req.user.dataValues.id } }).then(() => {
        ...
        return res.redirect('/unidadeprofile');

    }).catch(() => {
        return res.render('unidade/unidadeprofile/edit', { layout: 'main', profile: req.user.dataValues, data: dataForm, danger_msg: "Perfil não editado com sucesso!" });
    });
});
...
router.get('/edit-password', eAdmin, async (req, res) => {
    ...
    if (user) {    
        return res.render('unidade/unidadeprofile/edit-password', { layout: 'main', profile: req.user.dataValues });
    } else {
        req.flash("danger_msg", "Erro: Usuário não encontrado!");
        return res.redirect('/login');
    }
...
router.post('/edit-password', eAdmin, async (req, res) => {
    ...
    db.users.update(data, { where: { id: req.user.dataValues.id } }).then(() => {
        ...
        return res.render('unidade/unidadeprofile/edit-password', { layout: 'main', profile: req.user.dataValues, danger_msg: "Senha não editada" });
    });
...
router.post('/edit-image', upload.single('image'), eAdmin, async (req, res) => {
    ...
    db.users.update({ image: req.file.filename }, { where: { id: req.user.dataValues.id } }).then(() => {
        ...
        return res.render('unidade/unidadeprofile/edit-image', { layout: 'main', profile: req.user.dataValues, danger_msg: "Erro: Foto não editada!" });
    });
