    ...
    //---ENEM | FIM
            ...
            const ttServOf = await db.escalas.findAll({
                ...
                where: {
                    data_inicio: {
                        [Op.and]: [
                            Sequelize.where(Sequelize.fn('DATE_FORMAT', Sequelize.col('data_inicio'), '%b'), nomeMesConvert),
                            Sequelize.where(Sequelize.fn('YEAR', Sequelize.col('data_inicio')), nomeAno)
                        ]
                    },
...
router.get('/view/:id', eAdmin, async (req, res) => {
    ...
    if (pjes) {
        ...
        return res.render("unidade/unidadepjes/view", { layout: 'main', profile: req.user.dataValues, nomeMes, sidebarSituations: true, pjes, ttservMes, escalas: escalas.map(id => id.toJSON()), valorofdist, valorprcdist, valortotaldistribuido, cotaofexe, cotaprcexe, valortotalexecutado, id: id  });
        
    } else {
        // Criar a mensagem de erro
        req.flash("danger_msg", "Erro: PJES não encontrado!");
        // Redirecionar o usuário
        return res.redirect('/unidadepjes');
    }
});
...
router.get('/escalas', eAdmin, async (req, res) => {
    ...
    return res.render('unidade/unidadepjes/add', { layout: 'main', profile: req.user.dataValues, nomeMes, data: dataForm, sidebarSituations: true });
...
router.get('/deleteEscala/:id', eAdmin, async (req, res) => {
    ...
    db.escalas.destroy({
        ...
        return res.redirect('/unidade/unidadepjes/view/');
